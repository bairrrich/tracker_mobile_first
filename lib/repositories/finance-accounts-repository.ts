...
export class FinanceAccountsRepository {
  async getById(id: string): Promise<FinanceAccount | undefined> {
    return withDB((db) => db.financeAccounts.get(id)) ?? undefined
  }

  async getAll(): Promise<FinanceAccount[]> {
    return withDB((db) => db.financeAccounts.orderBy('createdAt').reverse().toArray()) ?? []
  }
  ...
  async getByType(type: AccountType): Promise<FinanceAccount[]> {
    const accounts = await withDB((db) => db.financeAccounts.where('type').equals(type).reverse().toArray()) || []
  ...
  async create(data: CreateAccountData): Promise<string> {
    ...
    await withDB((db) => db.financeAccounts.add({
  ...
  async markAsSynced(id: string): Promise<void> {
    await withDB((db) => db.financeAccounts.update(id, { synced: true }))
    const syncRecords = (await withDB((db) => db.syncQueue.where('[table+recordId]').equals(['finance_accounts', id]).and((record) => !record.synced).primaryKeys())) ?? []
