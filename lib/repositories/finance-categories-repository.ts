...
export class FinanceCategoriesRepository {
  async getById(id: string): Promise<FinanceCategory | undefined> { return withDB((db) => db.financeCategories.get(id)) ?? undefined }
  async getAll(): Promise<FinanceCategory[]> { return withDB((db) => db.financeCategories.orderBy('createdAt').toArray()) ?? [] }
  ...
  async getByType(type: CategoryType): Promise<FinanceCategory[]> {
    const categories = await withDB((db) => db.financeCategories.where('type').equals(type).toArray()) || []
  ...
  async initializePredefinedCategories(): Promise<void> {
    ...
    for (const cat of PREDEFINED_INCOME_CATEGORIES) {
      ...
      await withDB((db) => db.financeCategories.add({ id, name: cat.nameKey, type: 'income', isPredefined: true, icon: cat.icon, color: cat.color, createdAt: now, updatedAt: now, synced: false }))
    ...
    for (const cat of PREDEFINED_EXPENSE_CATEGORIES) {
      const id = generateUUID()
      await withDB((db) => db.financeCategories.add({ id, name: cat.nameKey, type: 'expense', isPredefined: true, icon: cat.icon, color: cat.color, createdAt: now, updatedAt: now, synced: false }))
  ...
  async create(data: CreateCategoryData): Promise<string> {
    const now = new Date()
    const id = generateUUID()
    await withDB((db) => db.financeCategories.add({ id, name: data.name, type: data.type, isPredefined: false, icon: data.icon, color: data.color, createdAt: now, updatedAt: now, synced: false }))
  ...
  async update(id: string, data: UpdateCategoryData): Promise<void> {
    ...
    await withDB((db) => db.financeCategories.update(id, { ...data, updatedAt: new Date() }))
  ...
  async delete(id: string): Promise<void> {
    ...
    await withDB((db) => db.financeCategories.update(id, { ...tombstone, synced: false }))
    await this.markForSync(id, 'delete', { id, deleted: true })
  }

  async hardDelete(id: string): Promise<void> { await withDB((db) => db.financeCategories.delete(id)) }

  private async markForSync(id: string, operation: 'insert' | 'update' | 'delete', data?: object): Promise<void> {
    await withDB((db) => db.syncQueue.add({ id: generateUUID(), table: 'finance_categories', recordId: id, operation, data: data ? JSON.stringify(data) : '', synced: false, createdAt: new Date() }))
  }

  async markAsSynced(id: string): Promise<void> {
    await withDB((db) => db.financeCategories.update(id, { synced: true }))
    const syncRecords = (await withDB((db) => db.syncQueue.where('[table+recordId]').equals(['finance_categories', id]).and((record) => !record.synced).primaryKeys())) ?? []
